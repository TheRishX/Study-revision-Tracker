...
export const AiAssistantDrawer: React.FC<AiAssistantDrawerProps> = ({
  ...
  const handleSendMessage = async (textToSend?: string) => {
    ...
    const activeTopics = videos.filter(video => video.status !== 'mastered').length;
    const replyText = activeTopics
      ? `You have ${activeTopics} active topic${activeTopics === 1 ? '' : 's'}. Choose one, study for 25 minutes without interruption, then write down three things you can recall without notes.`
      : 'Start with one small topic today. Study it for 25 minutes, then test yourself without looking at your notes.';
          ...
          <div className="p-4 sm:p-5 bg-amber-100 border-b-3 border-amber-950 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs font-bold text-amber-800">Private on-device revision coach</p>
              </div>
            </div>
            <button
              onClick={onClose}
