              ...
              {currentPage === 'overview' ? (
                ...
                <MernTopicsPage
                  ...
                  onCreateCategory={async name => {
                    const id = await addMainTopicCategory(name, mainTopicCategories.length);
                    const now = new Date().toISOString();
                  ...
                  onRenameCategory={async (oldName, newName) => {
                    const category = mainTopicCategories.find(item => item.name === oldName);
                    const affectedTopics = mainTopics.filter(item => item.category === oldName);
                    await Promise.all([
                      ...(category ? [updateMainTopicCategory(category.id, newName)] : []),
                      ...affectedTopics.map(item => updateMainTopic(item.id, { category: newName })),
                    ]);
                  ...
                  onDeleteCategory={async name => {
                    const category = mainTopicCategories.find(item => item.name === name);
                    const affectedTopics = mainTopics.filter(item => item.category === name);
                    await Promise.all([
                      ...(category ? [deleteMainTopicCategory(category.id)] : []),
                      ...affectedTopics.map(item => updateMainTopic(item.id, { category: 'Uncategorized' })),
                    ]);
                  ...
                  onReorderCategories={async orderedNames => {
                    const now = new Date().toISOString();
                    const reordered: MainTopicCategory[] = [];
                    for (let index = 0; index < orderedNames.length; index++) {
                      const name = orderedNames[index];
                      const existing = mainTopicCategories.find(category => category.name === name);
                      if (existing) {
                        await updateMainTopicCategoryOrder(existing.id, index);
                        reordered.push({ ...existing, orderIndex: index, updatedAt: now });
                      } else {
                        const id = await addMainTopicCategory(name, index);
                        reordered.push({ id, name, orderIndex: index, createdAt: now, updatedAt: now });
                      }
                    }
                    setMainTopicCategories(reordered);
                  }}
                  onMoveTopic={async (draggedId, targetId, placement) => {
                    const dragged = mainTopics.find(topic => topic.id === draggedId);
                    const target = mainTopics.find(topic => topic.id === targetId);
                    ...
                    while (changed) {
                      ...
                      mainTopics.forEach(topic => {
                        if (topic.parentId && (topic.parentId === dragged.id || descendants.has(topic.parentId)) && !descendants.has(topic.id)) { descendants.add(topic.id); changed = true; }
                      });
                    }
                    ...
                    descendants.forEach(id => {
                      const descendant = mainTopics.find(topic => topic.id === id);
                      if (descendant) updates.push({ id, parentId: descendant.parentId || '', category, orderIndex: descendant.orderIndex });
                    });
